          ...
          <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-1">
            {metrics.map((metric) => (
              <article key={metric.label} className="dark-card p-5">
                ...
                <p className="mt-2 text-xs font-semibold uppercase tracking-[0.24em] text-white/55">{metric.label}</p>
                {metric.source ? <p className="mt-3 text-sm text-white/55">{metric.source}</p> : null}
              </article>
            ))}
          </div>
        </div>
